...
export const getOrdersByUser = (userId: number) =>
  api.get<OrderResponse[]>(`/api/orders/user/${userId}`).then((r) => r.data);
export const createOrder = (data: OrderRequest) =>
  api.post<OrderResponse>("/api/orders", data).then((r) => r.data);
export const updateOrder = (id: number, data: OrderRequest) =>
  api.put<OrderResponse>(`/api/orders/${id}`, data).then((r) => r.data);
