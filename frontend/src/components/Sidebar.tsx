...
export function Sidebar({ collapsed, onToggle, mobileOpen, onMobileClose }: SidebarProps) {
  ...
  const sidebarContent = (
    ...
    >
      <Box sx={{ height: 64, display: 'flex', alignItems: 'center', justifyContent: collapsed ? 'center' : 'space-between', px: collapsed ? 1 : 2, borderBottom: '1px solid #e2e8f0', flexShrink: 0 }}>
        {!collapsed && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Box sx={{ width: 32, height: 32, borderRadius: '6px', background: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 700, color: '#fff' }}>R</Box>
            <Typography sx={{ fontSize: 16, fontWeight: 700, color: '#0f172a' }}>RetailOS</Typography>
          </Box>
        )}
        {collapsed && (
          <Box sx={{ width: 32, height: 32, borderRadius: '6px', background: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 700, color: '#fff' }}>R</Box>
        )}
        <IconButton onClick={onToggle} size="small" sx={{ display: { xs: 'none', md: 'flex' }, ml: collapsed ? 'auto' : 0 }}>
      ...
      <Box sx={{ p: 2, flexShrink: 0 }}>
        {collapsed ? (
          <Stack spacing={1} alignItems="center">
            <Avatar sx={{ width: 32, height: 32, bgcolor: '#e2e8f0', color: '#0f172a', fontSize: 14, fontWeight: 600 }}>
              {user?.name?.charAt(0) ?? '?'}
            </Avatar>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Avatar sx={{ width: 36, height: 36, bgcolor: '#e2e8f0', color: '#0f172a', fontSize: 14, fontWeight: 600 }}>
              {user?.name?.charAt(0) ?? '?'}
            </Avatar>
            <Box sx={{ flex: 1, minWidth: 0 }}>
              <Typography sx={{ fontSize: 13, fontWeight: 600, color: '#0f172a' }} noWrap>{user?.name}</Typography>
              <Typography sx={{ fontSize: 12, color: '#64748b' }} noWrap>{user?.email}</Typography>
            </Box>
  ...
  return (
    <>
      <Box sx={{ display: { xs: 'none', md: 'block' }, flexShrink: 0, width: w, transition: 'width 200ms ease' }}>
        <Box sx={{ position: 'fixed', top: 0, left: 0, height: '100vh', zIndex: 200 }}>
          {sidebarContent}
        </Box>
      </Box>
      {mobileOpen && (
        <Box ref={overlayRef} sx={{ display: { xs: 'flex', md: 'none' }, position: 'fixed', inset: 0, zIndex: 400, background: 'rgba(0,0,0,0.5)' }}>
          {sidebarContent}
        </Box>
      )}
    </>
