...
export default function CartPage() {
  ...
  const clearCart = () => {
    setCart([]); setAppliedCoupon(null); setDiscountAmount(0); setCouponInput('');
  };
  ...
  const applyCoupon = async () => {
    ...
    try {
      ...
      if (new Date(coupon.expiryDate) < new Date(today.getFullYear(), today.getMonth(), today.getDate())) { notify.error('This coupon has expired'); return; }
      if (coupon.usageLimit != null && coupon.usedCount >= coupon.usageLimit) { notify.error('Coupon usage limit reached'); return; }
      const discount = coupon.type === 'PERCENTAGE' ? (subtotal * Number(coupon.value)) / 100 : Math.min(subtotal, Number(coupon.value));
  ...
  const removeCoupon = () => { setAppliedCoupon(null); setDiscountAmount(0); setCouponInput(''); notify.info('Coupon removed'); };
  ...
  const buildAddress = () => {
    ...
    return [delivery.line1.trim(), delivery.landmark.trim(), `${delivery.city.trim()}, ${delivery.state.trim()} ${delivery.postalCode.trim()}`].filter(Boolean).join(', ');
  };
  ...
  return (
    <Box sx={{ maxWidth: 1000, mx: 'auto' }}>
      <Box sx={{ mb: 4 }}>
        <Typography variant="h5" sx={{ fontWeight: 700, color: '#0f172a' }}>Shopping Cart</Typography>
      </Box>
      ...
      {cart.length === 0 ? (
        <Paper elevation={0} sx={{ p: 6, textAlign: 'center', border: '1px solid #e2e8f0', borderRadius: '16px' }}>
          ...
          <Typography sx={{ fontSize: 18, fontWeight: 600, color: '#0f172a', mb: 1 }}>Your cart is empty</Typography>
          <Typography sx={{ color: 'text.secondary', mb: 3 }}>Looks like you haven't added anything to your cart yet.</Typography>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1.5fr 1fr' }, gap: 4 }}>
          {/* Cart Items */}
          <Box>
            <Typography sx={{ fontWeight: 600, mb: 2, fontSize: 16 }}>Items ({cart.reduce((a,c) => a + c.quantity, 0)})</Typography>
            <Stack spacing={2}>
              {cart.map(line => {
                const product = productMap[line.productId];
                if (!product) return null;
                return (
                  <Paper key={line.productId} elevation={0} sx={{ p: 2, border: '1px solid #e2e8f0', borderRadius: '12px', display: 'flex', gap: 2 }}>
                    <Box component="img" src={imageForProduct(product.id)} sx={{ width: 80, height: 80, borderRadius: '8px', objectFit: 'cover' }} />
                    <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                      <Typography sx={{ fontWeight: 600, color: '#0f172a', fontSize: 15 }}>{product.name}</Typography>
                      <Typography sx={{ color: 'text.secondary', fontSize: 14 }}>{formatINR(product.price)} each</Typography>
                    </Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                      <Stack direction="row" spacing={2} sx={{ alignItems: 'center', bgcolor: '#f8fafc', p: 1.5, borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                        <IconButton size="small" onClick={() => updateQty(product.id, line.quantity - 1)}><RemoveIcon fontSize="small" /></IconButton>
                        <Typography sx={{ fontWeight: 700, minWidth: 20, textAlign: 'center' }}>{line.quantity}</Typography>
                        <IconButton size="small" onClick={() => updateQty(product.id, line.quantity + 1)}><AddIcon fontSize="small" /></IconButton>
                      </Stack>
                      <Typography sx={{ fontWeight: 700, fontSize: 16, minWidth: 80, textAlign: 'right' }}>
            ...
            <Box sx={{ mt: 3, display: 'flex', justifyContent: 'flex-start' }}>
              <Button variant="text" color="error" onClick={clearCart} startIcon={<DeleteOutlineIcon />}>Clear Cart</Button>
            </Box>
          </Box>

          {/* Checkout Summary */}
          <Box>
            <Paper elevation={0} sx={{ p: 3, border: '1px solid #e2e8f0', borderRadius: '16px', position: 'sticky', top: 24 }}>
              <Typography sx={{ fontWeight: 600, mb: 2, fontSize: 16 }}>Order Summary</Typography>
              
              {/* Delivery Details */}
              <Box sx={{ mb: 3 }}>
                <Typography sx={{ fontSize: 12, fontWeight: 700, color: 'text.secondary', textTransform: 'uppercase', mb: 1.5 }}>Delivery Address</Typography>
                <Stack spacing={1.5}>
              ...
              <Box sx={{ mb: 3 }}>
                {appliedCoupon ? (
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 1.5, borderRadius: 2, bgcolor: '#f1f5f9' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <SellIcon fontSize="small" sx={{ color: 'primary.main' }} />
                      <Typography sx={{ fontSize: 14, fontWeight: 700 }}>{appliedCoupon.code}</Typography>
                    </Box>
                  ...
                  <Stack direction="row" spacing={1}>
                    ...
                    />
                    <Button variant="outlined" onClick={applyCoupon} disabled={couponLoading || !couponInput.trim()} sx={{ borderRadius: 1.5 }}>
                      Apply
                    </Button>
                  </Stack>
                )}
              ...
              <Stack spacing={1.5} sx={{ mb: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  <Typography sx={{ fontSize: 15, color: 'text.secondary' }}>Subtotal</Typography>
                  <Typography sx={{ fontSize: 15, fontWeight: 600 }}>{formatINR(subtotal)}</Typography>
                </Box>
                {discountAmount > 0 && (
                  <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Typography sx={{ fontSize: 15, color: 'text.secondary' }}>Discount</Typography>
                    <Typography sx={{ fontSize: 15, fontWeight: 600, color: 'success.main' }}>−{formatINR(discountAmount)}</Typography>
                  </Box>
                )}
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 1 }}>
                  <Typography sx={{ fontSize: 18, fontWeight: 700, color: '#0f172a' }}>Total Payable</Typography>
                  <Typography sx={{ fontSize: 20, fontWeight: 800, color: '#0f172a' }}>{formatINR(payable)}</Typography>
                </Box>
              </Stack>

              <Button variant="contained" fullWidth onClick={placeOrder} disabled={placingOrder} sx={{ py: 1.5, fontSize: 16, fontWeight: 700, borderRadius: '10px', textTransform: 'none', bgcolor: '#0f172a', '&:hover': { bgcolor: '#1e293b' } }}>
                Complete Checkout
              </Button>
            </Paper>
          </Box>
