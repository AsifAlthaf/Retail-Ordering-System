...
import {
  ...
  InputLabel, Link, OutlinedInput, Stack, TextField, Typography, Paper
} from '@mui/material';
...
const INITIAL_FORM: SignUpForm = {
  name: '', email: '', phone: '', address: '', city: '', state: '', postalCode: '', password: '', confirmPassword: '',
};
...
export default function SignUpPage() {
  ...
  return (
    <Box className="auth-bg">
      <Paper sx={{ width: '100%', maxWidth: 520, p: { xs: 3, md: 5 }, borderRadius: '12px' }}>
        ...
        <Typography variant="h5" sx={{ textAlign: 'center', mb: 3 }}>Create your account</Typography>

        <Collapse in={!!signupError} unmountOnExit>
          <Alert severity="error" onClose={() => setSignupError('')} sx={{ mb: 3 }}>{signupError}</Alert>
        </Collapse>

        <Box component="form" onSubmit={handleSubmit} noValidate>
          <Stack spacing={2}>
            <TextField label="Full name" value={form.name} onChange={e => setField('name', e.target.value)} error={!!fieldError('name')} helperText={fieldError('name')} fullWidth />
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
              <TextField label="Email" value={form.email} onChange={e => setField('email', e.target.value)} error={!!fieldError('email')} helperText={fieldError('email')} />
              <TextField label="Phone" value={form.phone} onChange={e => setField('phone', e.target.value.replace(/\D/g, '').slice(0, 10))} error={!!fieldError('phone')} helperText={fieldError('phone')} />
            </Box>
            <TextField label="Address line" value={form.address} onChange={e => setField('address', e.target.value)} error={!!fieldError('address')} helperText={fieldError('address')} fullWidth />
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr 120px', gap: 2 }}>
              ...
              <TextField label="Postal code" value={form.postalCode} onChange={e => setField('postalCode', e.target.value.replace(/\D/g, '').slice(0, 6))} error={!!fieldError('postalCode')} helperText={fieldError('postalCode')} />
            </Box>
            <Box>
              ...
              {form.password && (
                <Box sx={{ mt: 1 }}>
                  <Box sx={{ display: 'flex', gap: 0.5, mb: 0.5 }}>
                    {[1,2,3,4,5].map(i => <Box key={i} sx={{ flex: 1, height: 3, borderRadius: 2, background: i <= passwordStrength ? strengthColor : '#e2e8f0' }} />)}
                  </Box>
                  <Typography sx={{ fontSize: 11, color: strengthColor, fontWeight: 600, mb: 0.5 }}>{strengthLabel}</Typography>
                  <Stack spacing={0.25}>
                    {passwordRules.map(r => (
                      <Typography key={r.label} sx={{ fontSize: 11, color: r.met ? '#10b981' : '#64748b' }}>
                        {r.met ? '✓' : '○'} {r.label}
                      </Typography>
                    ))}
                  </Stack>
        ...
        <Typography sx={{ mt: 3, fontSize: 14, textAlign: 'center', color: 'text.secondary' }}>
          ...
          <Link component="button" type="button" onClick={e => { e.preventDefault(); navigate('/login'); }} sx={{ fontWeight: 600, color: 'primary.main', textDecoration: 'none' }}>
            Sign in
          </Link>
        </Typography>
      </Paper>
