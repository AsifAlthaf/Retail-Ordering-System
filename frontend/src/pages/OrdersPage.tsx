...
export default function OrdersPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setOrders(o.sort((a, b) => b.id - a.id)); setProducts(p);
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter(o => {
      ...
      const matchSearch = !q || String(o.id).includes(q) || o.deliveryAddress.toLowerCase().includes(q) || (o.couponCode ?? '').toLowerCase().includes(q);
      return matchStatus && matchSearch;
    });
  }, [orders, search, statusFilter]);

  const resetForm = () => setForm({ userId: 1, deliveryAddress: '', couponCode: '', status: 'PENDING', items: [{ productId: 0, quantity: 1, priceAtTime: 0 }] });
  const addItem    = () => setForm(f => ({ ...f, items: [...f.items, { productId: 0, quantity: 1, priceAtTime: 0 }] }));
  const removeItem = (i: number) => setForm(f => ({ ...f, items: f.items.filter((_, idx) => idx !== i) }));
  const updateItem = (i: number, patch: Partial<OrderItemRequest>) => setForm(f => ({ ...f, items: f.items.map((it, idx) => idx === i ? { ...it, ...patch } : it) }));

  const handleCreate = async () => {
    if (!form.deliveryAddress.trim()) { notify.warning('Delivery address required'); return; }
    try {
      setSaving(true);
    ...
    } catch { notify.error('Failed to create order'); }
    finally { setSaving(false); }
  };

  const viewDetails = async (orderId: number) => {
    try {
      setDetailLoading(true); setDetailDialog(true);
      setDetailOrder(await getOrderById(orderId));
    } catch { notify.error('Failed to load details'); setDetailDialog(false); }
    finally { setDetailLoading(false); }
  };

  const openStatusDialog = (o: OrderResponse) => { setStatusOrder(o); setNewStatus(o.status); setStatusDialog(true); };
    ...
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
        ...
        <Stack direction="row" spacing={1.5}>
          ...
          <Button variant="contained" onClick={() => { resetForm(); setCreateDialog(true); }}>New Order</Button>
        </Stack>
      </Box>

      {loading ? (
        <Stack spacing={1}><Skeleton height={60} variant="rounded" /><Skeleton height={60} variant="rounded" /></Stack>
      ) : (
          ...
          <Table>
            ...
            <TableBody>
              {filteredOrders.length === 0 ? (
                <TableRow><TableCell colSpan={6} align="center" sx={{ py: 4 }}>No orders found</TableCell></TableRow>
              ) : filteredOrders.map(o => (
                <TableRow key={o.id}>
                  <TableCell sx={{ fontWeight: 600 }}>#{o.id}</TableCell>
                  <TableCell><StatusBadge status={o.status} onClick={() => openStatusDialog(o)} /></TableCell>
      ...
      <Dialog open={detailDialog} onClose={() => setDetailDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          {detailLoading ? <Skeleton height={200} /> : detailOrder && (
            <Stack spacing={2} sx={{ mt: 1 }}>
              <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
                <Box><Typography variant="caption" color="text.secondary">Status</Typography><Box><StatusBadge status={detailOrder.status} /></Box></Box>
                <Box><Typography variant="caption" color="text.secondary">Total</Typography><Typography sx={{ fontWeight: 600 }}>{formatINR(detailOrder.totalAmount)}</Typography></Box>
                <Box><Typography variant="caption" color="text.secondary">Placed</Typography><Typography>{new Date(detailOrder.placedAt).toLocaleString()}</Typography></Box>
                <Box><Typography variant="caption" color="text.secondary">Delivery</Typography><Typography>{detailOrder.deliveryAddress}</Typography></Box>
              </Box>
              <Divider />
      ...
      <Dialog open={createDialog} onClose={() => setCreateDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            <TextField label="User ID" type="number" value={form.userId} onChange={e => setForm(f => ({ ...f, userId: parseInt(e.target.value) || 1 }))} fullWidth />
            <TextField label="Address" value={form.deliveryAddress} onChange={e => setForm(f => ({ ...f, deliveryAddress: e.target.value }))} fullWidth />
            <Divider>Items</Divider>
            {form.items.map((item, idx) => (
              <Stack key={idx} direction="row" spacing={1}>
                <FormControl size="small" fullWidth>
                  <InputLabel>Product</InputLabel>
                  <Select value={item.productId} onChange={e => { const pid = Number(e.target.value); updateItem(idx, { productId: pid, priceAtTime: products.find(p => p.id === pid)?.price ?? 0 }); }}>
                    {products.map(p => <MenuItem key={p.id} value={p.id}>{p.name}</MenuItem>)}
                  </Select>
                </FormControl>
                <TextField type="number" size="small" label="Qty" value={item.quantity} onChange={e => updateItem(idx, { quantity: parseInt(e.target.value) || 1 })} sx={{ width: 80 }} />
                <IconButton color="error" onClick={() => removeItem(idx)} disabled={form.items.length === 1}><RemoveCircleIcon /></IconButton>
              </Stack>
            ))}
            <Button startIcon={<AddCircleIcon />} onClick={addItem}>Add Item</Button>
          </Stack>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setCreateDialog(false)}>Cancel</Button>
          <Button variant="contained" onClick={handleCreate} disabled={saving}>Place Order</Button>
        </DialogActions>
      </Dialog>
      ...
      <ConfirmDialog
        open={deleteId !== null} title="Delete Order"
